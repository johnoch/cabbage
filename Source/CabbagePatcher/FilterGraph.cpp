...
{
	if(desc)
	CabbageUtils::showMessage(desc->name);
	else
	CabbageUtils::showMessage("bogus");
    if (desc != nullptr)
    {
        String errorMessage;

		//CabbagePluginAudioProcessor* instance;
		//instance = new CabbagePluginAudioProcessor("test.csd", false);
